    ...
    try:
        while True:
            try:
                ...
                print(f"Recieved Connection message: {message.decode()}")

                send_message = "Connection confirmed"
                client_sock.sendall(send_message.encode())
                while (1):
                    message = client_sock.recv(1024)
                    print(f"Recieved Connection message: Still connected")
                    configuration = parse_message(message.decode())

                    send_message = "Connection confirmed"
                    client_sock.sendall(send_message.encode())


                    operation = client_sock.recv(1024)


                    if operation.decode() == "1":
                        for i in range(len(hold)):
                            print(hold[i])
                        hold = []
                        display(configuration)
                        old_operation = operation.decode()

                    if operation.decode() == "2":
                        for i in range(len(hold)):
                            print(hold[i])
                        hold = []
                        display(configuration)
                    ...
                    if operation.decode() == "Pause":
                        fill(configuration, old_operation)
...
def fill(configuration, old_operation):
    global hold
    ...
    data2 = configuration["data2"]

    data3 = configuration["data3"]
    ...
    if str(command) == "PING":
        ping_address.append(str(data))
        ping_amount += 1

    if str(command) == "HTTP":
        http_address.append(str(data))
        http_amount += 1

    if str(command) == "HTTPS":
        https_address.append(str(data))
        https_amount += 1

    if str(command) == "NTP":
        ntp_address.append(str(data))
        ntp_amount += 1

    if str(command) == "DNS":
        dns_address.append(str(data))
        dns_query_type.append(str(data2))
        dns_record_type.append(str(data3))
        dns_amount += 1

    if str(command) == "TCP":
        tcp_address.append(str(data))
        tcp_port.append(str(data2))
        tcp_ammount += 1

    if str(command) == "UDP":
        udp_address.append(str(data))
        udp_port.append(str(data2))
        udp_ammount += 1


    while (1):
        if ping_amount != 0:
            print(Fore.GREEN + f"ID: {id} ICMP Data:" + Style.RESET_ALL)
            for i in range(len(ping_address)):
                ping_addr, ping_time = ping(ping_address[i])
                hold.append(time.strftime('%a %H:%M:%S') + f"Google DNS (ping): {ping_addr[0]} - {ping_time:.2f} ms" if (
                        ping_addr and ping_time is not None) else "Google DNS (ping): Request timed out or no reply received")
            ping_amount = 0

        if http_amount != 0:
            print(Fore.GREEN + f"ID: {id} HTTP Data:" + Style.RESET_ALL)
            for i in range(len(http_address)):
                http_server_status, http_server_response_code = check_server_http(http_address[i])
                hold.append(time.strftime('%a %H:%M:%S') +
                    f"HTTP URL: {http_address[i]}, HTTP server status: {http_server_status}, Status Code: {http_server_response_code if http_server_response_code is not None else 'N/A'}")

            http_amount = 0

        if https_amount != 0:
            print(Fore.GREEN + f"ID: {id} HTTPS Data:" + Style.RESET_ALL)
            for i in range(len(https_address)):
                https_url = https_address[i]
        ...
        if ntp_amount != 0:
            print(Fore.GREEN + f"ID: {id} NTP Data:" + Style.RESET_ALL)
            for i in range(len(ntp_address)):
                ntp_server = ntp_address[i]
                ntp_server_status, ntp_server_time = check_ntp_server(ntp_server)
                hold.append(time.strftime('%a %H:%M:%S') + f" {ntp_server} is up. Time: {ntp_server_time}" if ntp_server_status else f"{ntp_server} is down.")


            ntp_amount = 0

        if tcp_ammount != 0:
            print(Fore.GREEN + f"ID: {id} TCP Data:" + Style.RESET_ALL)
            for i in range(len(tcp_address)):
                tcp_port_server = tcp_address[i]
                tcp_port_number = int(tcp_port[i])
                tcp_port_status, tcp_port_description = check_tcp_port(tcp_port_server, tcp_port_number)
                hold.append(time.strftime('%a %H:%M:%S') +
                    f" Server: {tcp_port_server}, TCP Port: {tcp_port_number}, TCP Port Status: {tcp_port_status}, Description: {tcp_port_description}")

            tcp_ammount = 0

        if udp_ammount != 0:
            print(Fore.GREEN + f"ID: {id} UDP Data" + Style.RESET_ALL)
            for i in range(len(udp_address)):
                udp_port_server = udp_address[i]
                udp_port_number = int(udp_port[i])
                udp_port_status, udp_port_description = check_udp_port(udp_port_server, udp_port_number)
                hold.append(time.strftime('%a %H:%M:%S') +
                    f"Server: {udp_port_server}, UDP Port: {udp_port_number}, UDP Port Status: {udp_port_status}, Description: {udp_port_description}")

            udp_ammount = 0

        if dns_amount != 0:
            print(Fore.GREEN + f"ID: {id} DNS Data:" + Style.RESET_ALL)
            for i in range(len(dns_address)):
                dns_server_status, dns_query_results = check_dns_server_status(dns_address[i], dns_query_type[i],
                                                                               dns_record_type[i])
                hold.append(time.strftime('%a %H:%M:%S') +
                    f" DNS Server: {dns_address[i]}, Status: {dns_server_status}, {dns_record_type[i]} Records Results: {dns_query_results}")
...
def display(configuration):
    ...
    while (1):
        if ping_amount != 0:
            ...
            for i in range(len(ping_address)):
                ping_addr, ping_time = ping(ping_address[i])
                print(time.strftime('%a %H:%M:%S') + f"Google DNS (ping): {ping_addr[0]} - {ping_time:.2f} ms" if (
                    ping_addr and ping_time is not None) else "Google DNS (ping): Request timed out or no reply received")
            ping_amount = 0
        ...
        if ntp_amount != 0:
            ...
            for i in range(len(ntp_address)):
                ntp_server = ntp_address[i]
                ntp_server_status, ntp_server_time = check_ntp_server(ntp_server)
                print(time.strftime('%a %H:%M:%S') + f" {ntp_server} is up. Time: {ntp_server_time}" if ntp_server_status else f"{ntp_server} is down.")
